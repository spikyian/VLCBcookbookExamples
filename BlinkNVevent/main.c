...
void loop(void) {
    ...
    if (blinkOn) {
        ...
        digitalWrite(LED_CHANNEL, FALSE);   // Force OFF
    }
}
...
/*************************** Consumer Simple Service **************************/
Processed APP_processConsumedEvent(uint8_t tableIndex, Message * m) {
    // Check to see what to do with the event
    // In this minimal example we have the event stored and we use it to
    // enable or disable the blinking LED.
    blinkOn = !(m->opc & EVENT_ON_MASK);
    return PROCESSED;
}