...
static uint8_t removeTableEntry(uint8_t tableIndex) {
    ...
    writeNVM(EEPROM_NVM_TYPE, 0x100 + (sizeof(Event) + 1 + 1)*tableIndex + 0, 0x00);
    writeNVM(EEPROM_NVM_TYPE, 0x100 + (sizeof(Event) + 1 + 1)*tableIndex + 1, 0x00);
    writeNVM(EEPROM_NVM_TYPE, 0x100 + (sizeof(Event) + 1 + 1)*tableIndex + 2, 0x00);
    writeNVM(EEPROM_NVM_TYPE, 0x100 + (sizeof(Event) + 1 + 1)*tableIndex + 3, 0x00);
    writeNVM(EEPROM_NVM_TYPE, 0x100 + (sizeof(Event) + 1 + 1)*tableIndex + 4, 0x00);

    for (i=0; i<1; i++) {
        writeNVM(EEPROM_NVM_TYPE, 0x100 + (sizeof(Event) + 1 + 1)*tableIndex + (5 + i), 0x00);
    }
    flushFlashBlock();
    return 0;
}
    ...
    if (tableIndex == 0xff) {
        ...
        for (tableIndex=0; tableIndex<1; tableIndex++) {
            ...
            if (en == 0) {
                ...
                writeNVM(EEPROM_NVM_TYPE, 0x100 + (sizeof(Event) + 1 + 1)*tableIndex+1, nodeNumber&0xFF);
                writeNVM(EEPROM_NVM_TYPE, 0x100 + (sizeof(Event) + 1 + 1)*tableIndex+0, nodeNumber>>8);
                writeNVM(EEPROM_NVM_TYPE, 0x100 + (sizeof(Event) + 1 + 1)*tableIndex+3, eventNumber&0xFF);
                writeNVM(EEPROM_NVM_TYPE, 0x100 + (sizeof(Event) + 1 + 1)*tableIndex+2, eventNumber>>8);
                if (forceOwnNN) {
                    writeNVM(EEPROM_NVM_TYPE, 0x100 + (sizeof(Event) + 1 + 1)*tableIndex+4, 1);
                } else {
                    writeNVM(EEPROM_NVM_TYPE, 0x100 + (sizeof(Event) + 1 + 1)*tableIndex+4, 0);
                }
                for (e = 0; e < (sizeof(Event) + 1 + 1); e++) {
                    writeNVM(EEPROM_NVM_TYPE, 0x100 + (sizeof(Event) + 1 + 1)*tableIndex+5 +e, 0);
                }
                error = 0;
                break;
            }
...
uint8_t writeEv(uint8_t tableIndex, uint8_t evNum, uint8_t evVal) {
    ...
    writeNVM(EEPROM_NVM_TYPE, 0x100 + (sizeof(Event) + 1 + 1)*tableIndex+5 +evNum, evVal);
    return 0;
}
...
int16_t getEv(uint8_t tableIndex, uint8_t evNum) {
    ...
    return (uint8_t)readNVM(EEPROM_NVM_TYPE, 0x100 + (sizeof(Event) + 1 + 1)*tableIndex+5 +evNum);
}
...
uint8_t getEVs(uint8_t tableIndex) {
    ...
    for (evNum=0; evNum < 1; ) {
        uint8_t evIdx;
        for (evIdx=0; evIdx < (sizeof(Event) + 1 + 1); evIdx++) {
            evs[evNum] = (uint8_t)readNVM(EEPROM_NVM_TYPE, 0x100 + (sizeof(Event) + 1 + 1)*tableIndex+5 +evIdx);
            evNum++;
        }
    }
    return 0;
...
uint16_t getNN(uint8_t tableIndex) {
    ...
    flags = (uint8_t)readNVM(EEPROM_NVM_TYPE, 0x100 + (sizeof(Event) + 1 + 1)*tableIndex+4);
    if (flags & 1) {
        return nn.word;
    }
    lo = (uint8_t)readNVM(EEPROM_NVM_TYPE, 0x100 + (sizeof(Event) + 1 + 1)*tableIndex+1);
    hi = (uint8_t)readNVM(EEPROM_NVM_TYPE, 0x100 + (sizeof(Event) + 1 + 1)*tableIndex+0);
    return lo | (hi << 8);
}
...
uint16_t getEN(uint8_t tableIndex) {
    ...
    lo = (uint8_t)readNVM(EEPROM_NVM_TYPE, 0x100 + (sizeof(Event) + 1 + 1)*tableIndex+3);
    hi = (uint8_t)readNVM(EEPROM_NVM_TYPE, 0x100 + (sizeof(Event) + 1 + 1)*tableIndex+2);
    return lo | (hi << 8);
}
